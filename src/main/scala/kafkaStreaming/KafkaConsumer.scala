/**
 * Created by peter_v on 26/11/14.
 *
 * The initial version of this file is largely copied from
 *
 * https://github.com/stealthly/scala-kafka/blob/master/src/main/scala/KafkaConsumer.scala
 *
 * where it was distributed with this license:
 *
...
import kafka.consumer.{Consumer, Whitelist, ConsumerConfig}
...
case class KafkaConsumer(
  ...
  */
  ...
  println("setup:start topic=%s for zk=%s and groupId=%s".format(topic,zookeeperConnect,groupId))
  info("setup:start topic=%s for zk=%s and groupId=%s".format(topic,zookeeperConnect,groupId))
  val stream = connector.createMessageStreamsByFilter(filterSpec, 1, new DefaultDecoder(), new StringDecoder()).get(0)
  println("setup:complete topic=%s for zk=%s and groupId=%s".format(topic,zookeeperConnect,groupId))
